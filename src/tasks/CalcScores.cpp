#include "Task.hpp"

#include <fstream>

#include "utils/Assert.hpp"
#include "utils/FileStr.hpp"
#include "utils/Logger.hpp"
#include "Score.hpp"
#include "FrameGetter/VideoFrameReader.hpp"

using namespace std;

static void writeScoreToFile(const string& path, const Score& score, bool& coverAllow) {
    if (filesystem::is_regular_file(path) && !coverAllow) {
        AvLog("\"%s\"已存在，是否覆盖[y/a(全部覆盖)/n]:",path.c_str());
        fflush(stdin);
        switch(tolower(getchar())){
            case 'a': coverAllow=true;
            case 'y': break;
            case 'n':
            default: return;;
        }
    }
    ofstream of(path, ios::binary);
    int datalen = score.Dump();
    char* data=new char[datalen];
    score.Dump((uint8_t*)data);
    of.write(data, datalen);
    delete[] data;
}

//MSE计算
//TODO 支持多种像素格式计算
const uint16_t __square[]={65025,64516,64009,63504,63001,62500,62001,61504,61009,60516,60025,59536,59049,58564,58081,57600,57121,56644,56169,55696,55225,54756,54289,53824,53361,52900,52441,51984,51529,51076,50625,50176,49729,49284,48841,48400,47961,47524,47089,46656,46225,45796,45369,44944,44521,44100,43681,43264,42849,42436,42025,41616,41209,40804,40401,40000,39601,39204,38809,38416,38025,37636,37249,36864,36481,36100,35721,35344,34969,34596,34225,33856,33489,33124,32761,32400,32041,31684,31329,30976,30625,30276,29929,29584,29241,28900,28561,28224,27889,27556,27225,26896,26569,26244,25921,25600,25281,24964,24649,24336,24025,23716,23409,23104,22801,22500,22201,21904,21609,21316,21025,20736,20449,20164,19881,19600,19321,19044,18769,18496,18225,17956,17689,17424,17161,16900,16641,16384,16129,15876,15625,15376,15129,14884,14641,14400,14161,13924,13689,13456,13225,12996,12769,12544,12321,12100,11881,11664,11449,11236,11025,10816,10609,10404,10201,10000,9801,9604,9409,9216,9025,8836,8649,8464,8281,8100,7921,7744,7569,7396,7225,7056,6889,6724,6561,6400,6241,6084,5929,5776,5625,5476,5329,5184,5041,4900,4761,4624,4489,4356,4225,4096,3969,3844,3721,3600,3481,3364,3249,3136,3025,2916,2809,2704,2601,2500,2401,2304,2209,2116,2025,1936,1849,1764,1681,1600,1521,1444,1369,1296,1225,1156,1089,1024,961,900,841,784,729,676,625,576,529,484,441,400,361,324,289,256,225,196,169,144,121,100,81,64,49,36,25,16,9,4,1,0,1,4,9,16,25,36,49,64,81,100,121,144,169,196,225,256,289,324,361,400,441,484,529,576,625,676,729,784,841,900,961,1024,1089,1156,1225,1296,1369,1444,1521,1600,1681,1764,1849,1936,2025,2116,2209,2304,2401,2500,2601,2704,2809,2916,3025,3136,3249,3364,3481,3600,3721,3844,3969,4096,4225,4356,4489,4624,4761,4900,5041,5184,5329,5476,5625,5776,5929,6084,6241,6400,6561,6724,6889,7056,7225,7396,7569,7744,7921,8100,8281,8464,8649,8836,9025,9216,9409,9604,9801,10000,10201,10404,10609,10816,11025,11236,11449,11664,11881,12100,12321,12544,12769,12996,13225,13456,13689,13924,14161,14400,14641,14884,15129,15376,15625,15876,16129,16384,16641,16900,17161,17424,17689,17956,18225,18496,18769,19044,19321,19600,19881,20164,20449,20736,21025,21316,21609,21904,22201,22500,22801,23104,23409,23716,24025,24336,24649,24964,25281,25600,25921,26244,26569,26896,27225,27556,27889,28224,28561,28900,29241,29584,29929,30276,30625,30976,31329,31684,32041,32400,32761,33124,33489,33856,34225,34596,34969,35344,35721,36100,36481,36864,37249,37636,38025,38416,38809,39204,39601,40000,40401,40804,41209,41616,42025,42436,42849,43264,43681,44100,44521,44944,45369,45796,46225,46656,47089,47524,47961,48400,48841,49284,49729,50176,50625,51076,51529,51984,52441,52900,53361,53824,54289,54756,55225,55696,56169,56644,57121,57600,58081,58564,59049,59536,60025,60516,61009,61504,62001,62500,63001,63504,64009,64516,65025};
const uint16_t* _square=__square+255;
static double _getMSE(const AVFrame *f1, const AVFrame *f2){
    uint64_t mse=0;
    if (f1->format != f2->format) ThrowErr("两帧格式不同");
    switch (f1->format) {
    case AV_PIX_FMT_YUV420P: {
        #pragma omp parallel for reduction(+:mse)
        for (int i=0; i<f1->height ; ++i) {
            uint8_t* p1 = f1->data[0] + i*f1->linesize[0];
            uint8_t* p2 = f2->data[0] + i*f2->linesize[0];
            for (int j=0 ; j<f1->width ; ++j) {
                mse += _square[(short)p1[j]-(short)p2[j]];
            }
        }
        break;
    }
    default:
        ThrowErr("不支持的像素格式");
        break;
    }
    return (double)mse / (f1->width*f1->height);
}

static vector<double> CalcMSE(InputVideo& vd) {
    vector<double> result(1, 0);
    VideoFrameReader vfr(vd);
    HvFrame f1, f2;
    if (!vfr.nextFrame(f1)) ThrowErr("无法读取视频帧");
    while (vfr.nextFrame(f2) ){
        result.push_back(_getMSE(f1.fr, f2.fr));
        f1.swap(f2);
    }
    return result;
}

bool Task::_taskCalcScores(){
    string typeStr = tolower(args["type"].value_or("mse"));
    ScoreType type;
    bool coverAll = false;
    if (typeStr=="mse") type = ScoreType::MSE;
    else ThrowErr("未知指数类型");

    bool hasStatic = args["static"].has_value();
    bool hasCut = args["cut"].has_value();
    const double STATIC = hasStatic ? getReal("static") : 0;
    const double CUT = hasCut ? getReal("cut") : 0;

    AvLog("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n");
    AvLog("Task: Calc");
    if(hasStatic) AvLog("static:%lf\n", STATIC);
    if(hasCut) AvLog("cut:   %lf\n", CUT);
    AvLog("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n");

    for (auto& vd : inputs) {
        vd.print();
        vector <double> scoreValues;
        if (type == ScoreType::MSE) scoreValues = CalcMSE(vd);
        // else if();
        Score score(move(scoreValues), type);
        score.setFPS(av_q2d(vd.getInfo().fps));
        if (hasStatic) score.setStatic(STATIC);
        if (hasCut) score.setCut(CUT);
        writeScoreToFile(vd.getPath()+".scob", score, coverAll);
    }
    return true;
}